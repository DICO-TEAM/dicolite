...
async function fetchTokenInfo(currencyId, addr) {
  const isRawToken = bnToBn(currencyId).isZero();
  try {
    var [res, resTotal, resBalance] = await Promise.all([
      ...
      isRawToken ? api.derive?.balances?.all(addr) : api.query.tokens.accounts(addr, currencyId)
    ]);
    res = res.toJSON()
    if (res != null && res.metadata != null) {
      ...
      if (resBalance) {
        res.tokenBalance = isRawToken ? resBalance : resBalance.toJSON();
        if (isRawToken) {
          res.tokenBalance.free = resBalance?.availableBalance ?? 0;
          res.tokenBalance.frozen = resBalance?.lockedBalance ?? 0;
          res.tokenBalance.reserved = resBalance?.reservedBalance ?? 0;
        }
      }
      res.currencyId = isRawToken ? "0" : currencyId;
    }
    return res;
  } catch (error) {
    window.send("log", error.toString())
...
async function fetchFarmSupplyBalance(currencyId, pubKey, ss58) {
  ...
  var res;
  if (currencyId == 0) {
    res = await api.derive?.balances?.all(addr);
    if (res != null) {
