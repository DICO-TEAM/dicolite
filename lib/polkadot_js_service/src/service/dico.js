...
async function fetchTokenInfo(currencyId, addr) {
  ...
  try {
    var [res, resTotal, resBalance] = await Promise.all([
      ...
      isRawToken ? api.query.system.account(addr) : api.query.tokens.accounts(addr, currencyId)
    ]);
    res = res.toJSON()
    if (res != null && res.metadata != null) {
      ...
      if (resBalance) {
        res.tokenBalance = isRawToken ? resBalance.toJSON().data : resBalance.toJSON();
        if (isRawToken) {
          res.tokenBalance.frozen = bnToBn(res.tokenBalance?.miscFrozen ?? 0).add(bnToBn(res.tokenBalance?.feeFrozen ?? 0)).toString()
        }
      }
