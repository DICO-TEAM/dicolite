...
async function txFeeEstimate(txInfo, paramList) {
  try {
    ...
    if (txInfo.txName == "treasury_approveProposal") {
      tx = await gov.makeTreasuryProposalSubmission(paramList[0], false);
    } else if (txInfo.txName == "treasury_rejectProposal") {
      tx = await gov.makeTreasuryProposalSubmission(paramList[0], true);
    }else if (txInfo.txName == "dao_propose") {
      tx = await dico.makeDaoProposalSubmission(...paramList);
    } else {
  ...
  result.events
    ...
    .map(({ event: { data, method, section } }) => {
      if (section === "system" && method === "ExtrinsicFailed") {
        ...
        var str=`${section}.${method}`;
        if (str == "amm.Swapped"||str=="amm.LiquidityAdded"||str=="lbp.Swapped") {
          successRes=data;
        }
      }
    });
  return { success, error,successRes };
}
  ...
  return new Promise(async (resolve) => {
    ...
    const onStatusChange = (result) => {
      if (result.status.isInBlock || result.status.isFinalized) {
        const { success, error, successRes } = _extractEvents(api, result);
        if (success) {
          resolve({ hash: tx.hash.toString() , successRes});
        }
        if (error) {
          resolve({ error });
        }
